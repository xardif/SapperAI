package pl.edu.amu.wmi.sapper.encog;
...
public class OCR extends JFrame {
	 ...
	 */
	static final int DOWNSAMPLE_HEIGHT = 15;
	...
	OCR() {
		...
		setTitle("Parth OCR Neural Network");
		getContentPane().setLayout(null);
		setSize(405, 382);
		setVisible(false);
	...
	char[] mapNeurons() {
		...
		for (int i = 0; i < this.letterListModel.size(); i++) {
			final MLData input = new BasicMLData(DOWNSAMPLE_HEIGHT * DOWNSAMPLE_WIDTH);
			int idx = 0;
	...
	void recognize_actionPerformed(final java.awt.event.ActionEvent event) {
        ...
        for(int i = 0; i < this.letterListModel.size(); i++){
            SampleData sd = (SampleData) this.letterListModel.getElementAt(i);
            set.add(sd.getLetter());
        }



		JOptionPane
				.showMessageDialog(this, "  " + new ArrayList<>(set).get(best) + "   (Neuron #"
	...
	public void trainSOM() {
		...
		try {
			...
			for (int t = 0; t < this.letterListModel.size(); t++) {
                ...
                double[] idealSet = new double[set.size()];
                ...
                System.out.println(Arrays.toString(idealSet));
				trainingSet.add(new BasicMLDataPair(item, new BasicMLData(idealSet)));
			}


			this.network = new BasicNetwork();
			network.addLayer(new BasicLayer(null, true, DOWNSAMPLE_HEIGHT*DOWNSAMPLE_WIDTH));
			network.addLayer(new BasicLayer(new ActivationElliott(), true, 50));
			network.addLayer(new BasicLayer(new ActivationElliott(), false, set.size()));

			network.getStructure().finalizeStructure();
			network.reset();

			final Propagation train = new Backpropagation(network, trainingSet, 0.7, 0.3);
